...
//        提供了在数据库执行 SQL 命令所需的所有方法。你可以通过 SqlSession 实例来直接执行已映射的 SQL 语句。
    public static SqlSession getSqlSession(){
        return sqlSessionFactory.openSession(true);//我们就把sqlSession想象成操作数据库的对象。
    }
//    设置为TRUE：以后所有的东西都不用再手动commit了。比如我们在测试类中增删改测试类 都有“注意增删改需要提交事务：”的操作，对应代码为： sqlSession.commit();
//    现在设置为TRUE后，实现了工具类创建就实现自动提交事务！
